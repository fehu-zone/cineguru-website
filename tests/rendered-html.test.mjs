...
test("renders localized Turkish and English experiences", async () => {
  ...
  assert.match(trHtml, /<title>Cineguru Studio \| AI Destekli Video Prodüksiyon<\/title>/i);
  assert.match(trHtml, /application\/ld\+json/);
  assert.match(trHtml, /ProfessionalService/);
...
test("keeps real portfolio video identifiers in rendered output", async () => {
  ...
  for (const id of ["YgTxEe4HDbo", "qG5OExBLt8c", "YMRjZhdPsfk", "aZrWhEaHDBg", "7B39eWsDc5s"]) {
