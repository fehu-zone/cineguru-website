  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      frame = window.requestAnimationFrame(() => {
        ...
        document.querySelectorAll(".tc-display").forEach((element) => { element.textContent = timecode; });

        const services = serviceRef.current;
        if (services) {
          const start = services.offsetTop - window.innerHeight * 0.72;
          const end = services.offsetTop + services.offsetHeight - window.innerHeight * 0.32;
