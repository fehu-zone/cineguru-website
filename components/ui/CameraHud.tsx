...
export function CameraHud({ className, timecodeRef }: { className?: string; timecodeRef?: React.Ref<HTMLSpanElement> }) {
  const corner = "absolute size-[1.375rem] border-foreground/95 drop-shadow-md";
  ...
  return (
    <div className={cn("pointer-events-none absolute inset-0 z-10", className)} aria-hidden="true">
      ...
      <div className="absolute left-1.5 right-1.5 top-1.5 flex items-center justify-between font-mono text-[clamp(0.56rem,2.5vw,0.66rem)] font-semibold tracking-[0.1em] text-foreground/95">
      ...
      <div className="absolute bottom-1.5 left-1.5 right-1.5 flex items-center justify-between font-mono text-[clamp(0.56rem,2.5vw,0.66rem)] font-semibold tracking-[0.1em] text-foreground/95">
